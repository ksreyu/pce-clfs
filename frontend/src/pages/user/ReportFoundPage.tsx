...
export default function ReportFoundPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (form.storageLocation) formData.append('storageLocation', form.storageLocation);
      if (imageFile) formData.append('image', imageFile);

      await api.createFoundItem(formData);
      toast.success('Found item reported successfully! Help someone reunite with their belongings.');
      navigate('/my-items');
    } catch (err: any) {
  ...
  return (
    <div className="max-w-2xl mx-auto animate-fade-in">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>Report Found Item</h1>
          <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Help reunite someone with their lost item</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className={`rounded-xl shadow-sm p-6 space-y-6 transition-colors ${darkMode ? 'bg-gray-900 border border-gray-800' : 'bg-white border border-gray-100'}`}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className={`block text-sm font-medium mb-1 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Date Found *</label>
        ...
        <div>
          <label className={`block text-sm font-medium mb-1 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Location Found *</label>
        ...
        <div>
          <label className={`block text-sm font-medium mb-1 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Where is the item currently stored?</label>
        ...
        <button type="submit" disabled={loading}
          className="w-full bg-green-600 text-white py-3 rounded-lg font-medium hover:bg-green-700 transition disabled:opacity-50 disabled:cursor-not-allowed active:scale-95">
